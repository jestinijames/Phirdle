...
import SixLetterWordle from '@/components/SixLetterWordle';

const SixLetterWord = ({ team }: { team: string }) => {
  const [solution, setSolution] = React.useState(null);

  React.useEffect(() => {
    const fetchRandomSolution = async () => {
      const solutions = await fetch(
        'http://localhost:3002/sixLetterWords'
      ).then((res) => res.json());
  ...
  return (
    <Header>
      <div className='rounded-lg border bg-[#f5f5f9} shadow-sm'>
        {solution && <SixLetterWordle solution={solution} team={team} />}
      </div>
    </Header>
  );
};

export default SixLetterWord;
